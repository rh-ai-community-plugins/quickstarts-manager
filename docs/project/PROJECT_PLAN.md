...
1. **In-repo chart fetching** (issue #13): For quickstarts using `chart.type: repo`, the BFF fetches only the chart subdirectory via the **GitHub Git Trees + Blobs API** (`bff/src/utils/githubChart.ts`) rather than downloading a full repo tarball. The recursive tree is listed once, blob paths under `chart.path` are filtered, and each blob is fetched and written to a temp directory (cleaned up after the Helm operation). Includes rate-limit detection (`X-RateLimit-Remaining`/`-Reset`), optional `GITHUB_TOKEN` auth, and a configurable API base via `GITHUB_API_BASE` (or derived from the repo host). Note: this covers **chart fetching only** — the registry (`quickstarts.yaml`) and metadata (`quickstart.yaml`) are still fetched from `raw.githubusercontent.com` (github.com only), so full GitHub Enterprise support is not wired end-to-end. This is intentional: quickstarts are meant to live on public github.com. If enterprise/self-hosted registries are needed later, switch the registry + metadata clients to the Contents API (`{apiBase}/repos/{owner}/{repo}/contents/{path}`) reusing `parseRepoUrl()` from `bff/src/utils/githubChart.ts`; keep `raw` for github.com to avoid spending REST API rate-limit quota.
