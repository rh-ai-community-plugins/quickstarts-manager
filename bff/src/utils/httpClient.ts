...
export function fetchUrl(url: string, options: FetchOptions | number = {}): Promise<string> {
  const opts: FetchOptions = typeof options === 'number'
    ? { redirectsRemaining: options }
    : options;
  const redirectsRemaining = opts.redirectsRemaining ?? MAX_REDIRECTS;
